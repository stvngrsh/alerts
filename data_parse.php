		...
		if($h2->item(0) != NULL){
			...
			if($a->item(0) != NULL){
				...
				if(0 === strpos($title, 'UMD Safety Notice')){
					...
					$description = mysqli_real_escape_string($db, trim(str_replace("\xc2\xa0",' ',trim($output_array[1][0]))));
					...
					$stmt = mysqli_prepare($db, "INSERT INTO `alerts` (`incident`, `offcampus`, `date`, `time_one`, `time_two`, `location`, `case`, `description`) VALUES ('$incident', '$offcampus', '$o_date', '$time_one', '$time_two', '$location', '$case', '$description')") or die(mysqli_error($db));
